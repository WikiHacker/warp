...
checkTun(){
    if [[ ! $TUN =~ "in bad state"|"处于错误状态"|"ist in schlechter Verfassung" ]]; then
        if [[ $VIRT == lxc ]]; then
            ...
            wget -N --no-check-certificate https://raw.githubusercontent.com/wikihacker/warp/main/tun.sh && bash tun.sh
        else
            red "检测到未开启TUN模块, 请到VPS后台控制面板处开启"
            exit 1
        fi
...
installWgcf(){
    ...
    if [[ $SYSTEM == "CentOS" ]]; then
        ...
        if [[ $OSID == 9 ]] && [[ -z $(type -P resolvconf) ]]; then
            wget -N https://raw.githubusercontent.com/wikihacker/warp/main/resolvconf -O /usr/sbin/resolvconf
    ...
    if [[ $main -lt 5 ]] || [[ $minor -lt 6 ]] || [[ $VIRT =~ lxc|openvz ]]; then
        wget -N --no-check-certificate https://raw.githubusercontent.com/wikihacker/warp/main/wireguard-go-$(archAffix) -O /usr/bin/wireguard-go
        chmod +x /usr/bin/wireguard-go
    fi
    
    wget -N --no-check-certificate https://raw.githubusercontent.com/wikihacker/warp/main/wgcf_2.2.15_linux_$(archAffix) -O /usr/local/bin/wgcf
...
installWireProxy(){
    ...
    wget -N https://raw.githubusercontent.com/wikihacker/warp/main/wireproxy-$(archAffix) -O /usr/local/bin/wireproxy
...
warpnf(){
    ...
    case "$clientInput" in
        1 ) wget -N --no-check-certificate https://raw.githubusercontent.com/wikihacker/warp/main/netflix4.sh && bash netflix4.sh ;;
        2 ) wget -N --no-check-certificate https://raw.githubusercontent.com/wikihacker/warp/main/netflix6.sh && bash netflix6.sh ;;
        3 ) wget -N --no-check-certificate https://raw.githubusercontent.com/wikihacker/warp/main/netflixcli.sh && bash netflixcli.sh ;;
        4 ) wget -N --no-check-certificate https://raw.githubusercontent.com/wikihacker/warp/main/netflixwire.sh && bash netflixwire.sh ;;
    esac
}
